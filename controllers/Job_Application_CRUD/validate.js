  ...
  try {
    ...
    if (
      ...
      if(data.id !== ""){
          ...
          for(let i = 0; i < data.Sname.length; i++){
            if(data.Sname[i] !== ""){
              let uped = `update education_details set education_type='${data.etype[i]}',name_of_school_or_university='${data.Sname[i]}',course_name='${data.Course[i]}',passing_year='${data.Year[i]}',persentage='${data.Percentage[i]}' where id = '${data.eid[i]}';`
          ...
          if (typeof data.PreferedLocation === "object") {
            for (let i = 0; i < data.PreferedLocation.length; i++) {
              if (data.PreferedLocation[i] !== "") {
                if (data.NP === "" || data.cctc === "") {
                  let PDsql = "";
                  PDsql = `update preference_details set prefered_location='${data.PreferedLocation[i]}',department='${data.dprt}',expacted_CTC=${data.ectc} where id = '${Number((data.pid).split(',')[i])}';`;
                  let [PDresult] = await connection.query(PDsql);
                } else {
                  let PDsql = "";
                  PDsql = `update preference_details set prefered_location='${data.PreferedLocation[i]}',department='${data.dprt}',expacted_CTC=${data.ectc}, notice_period='${data.NP}',current_CTC='${data.cctc}' where id = ${Number((data.pid).split(',')[i])};`;
                  let [PDresult] = await connection.query(PDsql);
                }
              }
            }
            ...
            if (data.PreferedLocation !== "") {
              if (data.NP === "" || data.cctc === "") {
                let PDsql = "";
                PDsql = `update preference_details set prefered_location='${data.PreferedLocation}',department='${data.dprt}',expacted_CTC=${data.ectc}, notice_period='${data.NP}',current_CTC='${data.cctc}' where id = ${data.pid};`;
                let [PDresult] = await connection.query(PDsql);
              } else {
                let PDsql = "";
                PDsql = `update preference_details set prefered_location='${data.PreferedLocation}',department='${data.dprt}',expacted_CTC=${data.ectc} notice_period='${data.NP}',current_CTC='${data.cctc}' where id = ${data.pid};`;
          ...
          return res.redirect("/userDetails/generate");
      }
      else{
        ...
        for (let i = 0; i < data.Sname.length; i++) {
          if (data.Sname[i] !== "") {
            let EDsql = `insert into education_details(cid,education_type,name_of_school_or_university,course_name,passing_year,persentage) values (${cid},"${data.etype[i]}", "${data.Sname[i]}", "${data.Course[i]}", "${data.Year[i]}", "${data.Percentage[i]}");`;
